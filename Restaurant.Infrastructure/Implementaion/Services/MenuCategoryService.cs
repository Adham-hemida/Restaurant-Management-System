...
public class MenuCategoryService(IMenuCategoryRepository menuCategoryRepository, IHttpContextAccessor httpContextAccessor) : IMenuCategoryService
{
	...
	public  async Task<IEnumerable<MenuCategoryBasicResponse>> GetAllAsync( CancellationToken cancellationToken)
	{
		return await  _menuCategoryRepository
			.GetAsQueryable()
			.Where(x => x.IsActive)
			.AsNoTracking()
			.ProjectToType<MenuCategoryBasicResponse>()
	...
	public async Task<Result<MenuCategoryBasicResponse>> UpdateAsync(int id,MenuCategoryRequest request, CancellationToken cancellationToken)
	{
		var httpRequest = _httpContextAccessor.HttpContext?.Request;
		var origin = $"{httpRequest?.Scheme}://{httpRequest?.Host}";
		...
		if (menuCategoryIsExist)
			return Result.Failure<MenuCategoryBasicResponse>(MenuCategoryErrors.DuplicatedMenuCategory);


		var menuCategory = await _menuCategoryRepository.GetByIdAsync(id, cancellationToken);

		if (menuCategory is null)
			return Result.Failure<MenuCategoryBasicResponse>(MenuCategoryErrors.MenuCategoryNotFound);

	    menuCategory = request.Adapt(menuCategory);
		await _menuCategoryRepository.UpdateAsync(menuCategory, cancellationToken);
		
			
		return Result.Success(menuCategory.Adapt<MenuCategoryBasicResponse>());
	}
}
